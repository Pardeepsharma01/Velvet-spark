  ...
  return (
    <div className="flex items-center gap-2 sm:gap-4">
      ...
      <div className="flex items-center gap-1">
        ...
        <Link href="/cart" className="relative h-10 w-10 flex items-center justify-center rounded-full text-foreground/70 hover:text-gold hover:bg-gold/10 transition-colors duration-200">
          <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5">
      ...
      <div className="hidden sm:block">
        {loading ? (
          ...
          <Link href="/profile" className="flex h-9 w-9 items-center justify-center rounded-full border border-primary/30 bg-primary/5 text-xs font-bold text-primary hover:bg-primary/10">
            {initials}
          </Link>
        ) : (
          <Link href="/auth/login" className="hidden sm:inline-flex items-center justify-center h-9 px-4 rounded-lg border border-gold text-gold text-sm font-medium hover:bg-gold hover:text-white transition-all duration-200">Sign In</Link>
        )}
      </div>
    ...
    <div className="mt-auto p-6 bg-gold/5">
      {isLoggedIn ? (
        <Link href="/profile" className="flex items-center gap-3 p-4 rounded-xl border border-gold/20 bg-white dark:bg-charcoal/50">
          ...
          <span className="font-bold text-charcoal">My Account</span>
        </Link>
      ) : (
        <Link 
          href="/auth/login"
