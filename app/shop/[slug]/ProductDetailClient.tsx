...
//             </div> */}
//          // Yahan img ek object hai, isliye hum img.url use karenge
// <div className="relative w-full h-full bg-ivory dark:bg-charcoal-600">
//   <Image
//     src={img.url} // Agar property ka naam 'url' hai
...
//           //   <span className="text-gold/40 text-lg">✦</span>
//           // </div>
//         <div className="w-16 h-16 md:w-20 md:h-20 rounded-lg border-2 border-border bg-charcoal-100 flex items-center justify-center text-xs text-charcoal-400">
//   No Image
// </div>
//         )}
//       </div>
...
//       {images[activeIndex] ? (
//     <Image
//       src={images[activeIndex].url} // Dynamic URL yahan se aayega
//       alt={name}
//       fill
//       priority
...
function ImageGallery({
  ...
  return (
    <div className="flex flex-col-reverse md:flex-row gap-3">
        ...
        {images.length > 0 ? (
          images.map((img, i) => (
            <button
              ...
              onClick={() => setActiveIndex(i)}
              className={`
              flex-shrink-0 w-16 h-16 md:w-20 md:h-20 rounded-lg border-2 overflow-hidden transition-all duration-200 relative
              ${i === activeIndex ? "border-gold" : "border-border hover:border-gold/40"}
            `}
            >
              <div className="relative w-full h-full bg-ivory dark:bg-charcoal-600">
      ...
      {/* Main image with crossfade — FIXED BOUNDARIES */}
      <div className="flex-1 aspect-square relative rounded-xl bg-ivory dark:bg-charcoal-600 border border-border overflow-hidden">
        <AnimatePresence mode="wait">
          <motion.div
            key={activeIndex}
            initial={{ opacity: 0 }}
...
export default function ProductDetailClient({
  ...
  const accordionItems = [
    ...
    },
    {
      title: "Shipping & Returns",
      content:
        "Free shipping on orders above ₹999. Standard delivery: 5-7 business days. Express delivery: 2-3 business days. Easy 7-day return policy — no questions asked.",
    },
  ];
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12">
        ...
        <div className="flex flex-col gap-6">
          ...
          <FadeIn delay={0.1}>
            <div className="flex items-center gap-2 flex-wrap">
              {product.isNewArrival && (
                <span className="px-2.5 py-1 rounded-md bg-gold text-white text-2xs font-bold uppercase tracking-wider">
                  New Arrival
                </span>
              )}
              {product.isBestseller && (
                <span className="px-2.5 py-1 rounded-md bg-charcoal text-ivory text-2xs font-bold uppercase tracking-wider">
                  Bestseller
                </span>
              )}
              {hasDiscount && (
                <span className="px-2.5 py-1 rounded-md bg-rose-gold text-white text-2xs font-bold uppercase tracking-wider">
          ...
          <FadeIn delay={0.45}>
            <div className="hidden sm:flex flex-row gap-3 mt-2">
              ...
              <Button
                ...
                size="md"
                onClick={() => {
                  handleToggleWishlist(product, pathname);
              ...
              >
                ...
                {/* {addedToWishlist ? "✓ Added To Wishlist" : "♡ Wishlist"} */}
              </Button>
            </div>
          </FadeIn>
